...
while true; do
  ...
  if [[ ! -f ${ACME_BASE}/acme/key.pem ]]; then
    ...
    openssl genpkey -algorithm EC -pkeyopt ec_paramgen_curve:P-256 -pkeyopt ec_param_enc:named_curve > ${ACME_BASE}/acme/key.pem
  else
    log_f "Using existing domain ecdsa key ${ACME_BASE}/acme/key.pem"
  fi
